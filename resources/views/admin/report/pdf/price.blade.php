<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <title>Laporan Harga Produk</title>
    <meta http-equiv="Content-Type" content="charset=utf-8" />
    <link rel="stylesheet" href="{{ public_path('css') }}/pdf/bootstrap.min.css" media="all" />
    <style>
        body {
            font-family: Arial, sans-serif;
            font-size: 11px;
        }

        hr {
            margin: 1px;

            border: none;
            border-top: 2px solid #000;
        }

        tr {
            margin: 0 !important;
            padding: 0 !important;
        }

        .table-custom {
            border-collapse: collapse;
            width: 100%;
        }

        .table-custom tr,
        .table-custom th,
        .table-custom td {
            padding-left: 5px;
            border: 1px solid black;
        }
    </style>
    <link href="{{ public_path('img/logo.png') }}" rel="icon" type="image/png">
    {{-- <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.6.3/css/font-awesome.min.css"> --}}
</head>

<body>
    <main>
        <table style=" width:100%; margin-bottom:10px;">
            <tr>
                <td style="width: 20%" class="text-center">
                    <img style="width: 80px;" src="{{ public_path('img') }}/logo.png">
                </td>
                <td class="text-center" style="width: 80%">
                    <b>
                        <p class="m-0" style="font-size: 14px;"><b>AVIANTARA GROUP</b></p>
                        <i style="font-size: 10px;">Jalan Husen palela</i>
                    </b>
                </td>
                <td style="width: 20%"></td>
            </tr>
        </table>
        <hr>
        <p>
            <b>Laporan : </b> Harga Produk<br>
            <b>Gudang : </b> {{ $wirehouse }}<br>
        </p>
        <table class="table-custom">
            <thead>
                <tr>
                    <th>No</th>
                    <th>Produk</th>
                    <th>Gudang</th>
                    <th>Isi Produk</th>
                    <th>Harga Modal</th>
                    <th>Harga Grosir</th>
                    <th>Keuntungan</th>
                    <th>%</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($data as $item)
                    @php
                        //price origin
                        $price_origin = App\Models\ProductStok::where('id_product', $item->id)
                            ->where('type', 'Masuk')
                            ->orderByDesc('id')
                            ->first();
                        $modal = $price_origin != null ? $price_origin->price_origin : 0;
                        //price grosir
                        $price_grosir = App\Models\ProductPrice::where('id_product', $item->id)
                            ->orderByDesc('id')
                            ->first();
                        $grosir = $price_grosir != null ? $price_grosir->price_grosir : 0;
                        //price fee
                        $price_fee = $modal > 0 ? $grosir - $modal : 0;
                        //presentase
                        $presentase = $modal > 0 ? (($grosir - $modal) / $modal) * 100 : 0;
                    @endphp
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $item->name }}</td>
                        <td>{{ $item->wirehouse->name }}</td>
                        <td>{{ $item->quantity_unit . ' ' . $item->sub_unit . ' /' . $item->unit }}</td>
                        <td>Rp {{ number_format($modal) }}</td>
                        <td>Rp {{ number_format($grosir) }}</td>
                        <td>Rp {{ number_format($price_fee) }}</td>
                        <td>{{ $presentase }}%</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    </main>
</body>

</html>
